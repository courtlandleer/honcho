...
| `GRANOLA_EXTERNALS` | What other participants become: `full` (peers with derived representations, default), `store` (peers whose messages are stored and searchable but not reasoned over), or `none` (no peers for externals — only your side and summaries are imported) |
| `GRANOLA_EARLIEST` | Start of the history window, `YYYY-MM-DD` (default: `2024-01-01`) |
| `aliases.json` | Optional identity registry next to the script: maps each canonical peer ID to the emails/names a source may report, so the same person resolves to one peer across meetings and across integrations |
...
| Individual meeting | Session (`meeting-{id}`) | One session per meeting; title/date/mode/source on session metadata |
...
Real contacts rarely have exactly one identifier, though — the same person shows up as a work email in meetings, a personal email in threads, or just a display name on an invite. For that, drop an `aliases.json` next to the script:
...
Every identifier on the right resolves to the canonical peer ID on the left. The script also stamps each peer's metadata with the emails, names, and sources observed for it, so other importers can resolve identities against the same registry.

### Auto-Detecting "Me"

The script asks Granola which account you're logged in as (`get_account_info`) and anchors "me" to that email. Granola also marks a note's creator with `(note creator)`, and for meetings you recorded the two agree — so you're identified automatically, no configuration needed. If you want your messages attributed to an existing peer in your workspace instead of the email-derived ID, set `GRANOLA_ME_PEER`.
...
Anchoring to the account (not just the `(note creator)` tag) matters because Granola notes can be **created by a teammate and shared into your account**. In those, the `Me:` track is *their* microphone, not yours — blindly trusting the tag would write a colleague's spoken words onto your peer. When the creator isn't you, the script flags the meeting and skips it by default rather than corrupt your representation (you can still import its summary as your own note).
...
### Multi-Person Calls: Your Turns + the Summary

Granola's transcript uses `Them:` for all non-creator speakers with no disambiguation — in a 4-person call, everyone else is just `Them:`. But `Me:` is different: it's audio from your own microphone, so your words are attributable with high confidence no matter how many people were on the call — and fragmenting *your* representation is the worst outcome. So multi-person meetings import in `multi` mode: your turns become your messages, Granola's summary becomes the shared record, and the other attendees join the session as members, letting Honcho observe their presence at summary fidelity without ever putting collapsed `Them:` speech in any individual's mouth.
...
Speaker labels come from audio routing, and participant lists come from calendar invites — both can mislead. The review flow flags the suspicious cases and suggests a safe resolution:

- **Note created by someone else** — a teammate's shared note, where `Me:` is their microphone. Suggested: skip (or import the summary as your own note); never written to your peer as if you said it.
- **Attendees listed but zero `Them:` turns** — likely an in-person meeting recorded through your laptop mic, meaning `Me:` may contain other people's voices. Suggested: import the summary only (override available).
- **Someone spoke but no attendees are listed** — an ad-hoc call with no invite. Suggested: import your turns + summary; or pick who `Them` is.
- **Multi-person call that was actually a 1:1** (extra invitees who never spoke) — you can override and select the actual speaker for full attribution.

Calendar artifacts that aren't people — meeting-room resources and shared calendars (`...@resource.calendar.google.com`, `...@group.calendar.google.com`) — are dropped automatically, so they never become peers.

### Interactive Confirmation

For each meeting, you choose the import mode: two-person (full attribution), multi (your turns + summary), summary only, or skip — or press `a` once to accept the suggested defaults for everything remaining.
